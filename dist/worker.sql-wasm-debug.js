...
Module["onRuntimeInitialized"] = function onRuntimeInitialized() {
  ...
  var sqlite3_aggregate_context = cwrap("sqlite3_aggregate_context", "number", [
    "number",
    "number",
  ]);
   ...
   * @param {boolean} isFromFile whether init from file or not
   * @param {string} databaseFolder name of folder
   * @param {string} databaseName name of database
   * an SQLite database file
   */
  function Database(data, isFromFile, databaseFolder, databaseName) {
    console.log('create database',isFromFile,databaseFolder,databaseName)
    if (isFromFile === false) {
      this.filename = databaseFolder + "/" + databaseName;
      this.memoryFile = true;
      if (data != null) {
        FS.createDataFile(databaseFolder, databaseName, data, true, true);
      }
    } else {
      this.fileName = data;
    }
